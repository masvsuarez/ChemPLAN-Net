...
if __name__ == '__main__':
    ...
    train_FFAll = np.vstack((train_FFAll, test_FFAll))
    train_b = np.hstack((train_b, test_b))
    train_nb = np.hstack((train_nb, test_nb))
    print("loaded data")
...
#     test_nb = test_nb[selTest]
    ...
    np.random.seed(0)
    ss = np.random.choice(range(train_FFAll.shape[0]), int(0.001*train_FFAll.shape[0]), replace=False)
    ...
    end3 = time.time()
    print("val data arrays2tensor: ", end3-start3)
...
#         loaded_state_dict = temp
    ...
    for epoch in range(args.epochs):
        ...
        if state['val_loss'] < best_accuracy:
            best_accuracy = state['val_loss']
