...
# Option 2: testing HIV_Protease f_pocket&FEATURE output

AQ = pickle.load(open("%s/AllQuery.df" %args.datadir, "rb"))
AQ_environments = AQ.shape[0]
AQ_iter = AQ_environments//150 + 1
AQ = AQ.iloc[:,8:].values.astype(float)

for iteration in range(AQ_iter):
    testing = AQ[iteration*150:(iteration+1)*150]
    ...
    if iteration < AQ_iter-1:
        pickle.dump(coll, open("%s/%s_Probabilities_noEnv%s_%s.mat"%(args.datadir, args.name, iteration*150, (iteration+1)*150), "wb"))
    else:
        pickle.dump(coll, open("%s/%s_Probabilities_noEnv%s_FIN.mat"%(args.datadir, args.name, iteration*150), "wb"))
